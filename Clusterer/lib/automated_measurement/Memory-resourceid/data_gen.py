...
def record_system_load(data_points, reqs, timeout):
    ...
    proc = Popen(['docker', 'stats'], stdout=f)
    try:
...
def compose_up(timeout):
    proc = Popen(['docker-compose', 'up'], cwd='/Users/base/MEGA/Universität/Tez/DataWiz')
    try:
        outs, errs = proc.communicate(timeout=timeout)
    except TimeoutExpired:
        proc.kill()
...
if __name__ == "__main__":
    num_of_requests = [str(el) for el in list(range(1, 247, 2))]
    secs = 5 * 60  # 5 min

    data_points = '500'
    reqs = 1
    while True:
    # for reqs in num_of_requests:
        try:
            timeout = time() + secs
            ...
            recorder = multiprocessing.Process(
                target=record_system_load, args=(data_points, str(reqs), secs, ))
            recorder.start()
            ...
            print_exc()
    create_visualisation_command = 'python3 cpu_mem_reader.py --intervals ' + ' '.join(num_of_requests)
    call(shlex.split(create_visualisation_command))
