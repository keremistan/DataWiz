...
def visualise_results(all_extracted_vals):
    cpu_means, mem_means = get_means(all_extracted_vals)
    cpu_df = pd.DataFrame({'x': [1, 5, 10, 20, 50], '500_points': cpu_means['cpu_500'], '1000_points': cpu_means['cpu_1000']})
    ...
    plt.figure()
    cpu_df = pd.DataFrame({'x': [1, 5, 10, 20, 50], '500_points': mem_means['mem_500'], '1000_points': mem_means['mem_1000']})
    plt.plot('x', '500_points', data=cpu_df)
    plt.plot('x', '1000_points', data=cpu_df)
    plt.xlabel('Simultaneous Requests')
...
def get_means(all_extracted_vals):
    ...
    for cpu in ['cpu_500', 'cpu_1000']:
        for curr_req in all_extracted_vals[cpu]:
            mean_curr_req = np.mean(curr_req) * 2 # redis and clusterer are one system, not two.
...
if __name__ == "__main__":
    ...
    visualise_results(all_extracted_vals)